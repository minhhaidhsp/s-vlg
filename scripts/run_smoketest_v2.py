...
def main() -> None:
    ...
    if early_stopper is not None and early_stopper.best_epoch is not None:
        ...
        # Shared with train()'s --resume-from path: handles the bitsandbytes 4-bit
        # quantization-metadata key mismatch (frozen base Qwen weights) that a plain
        # strict=True load_state_dict spuriously fails on -- see checkpoint_utils.py.
        restore_model_and_optimizer(best_ckpt, full_model)
